...
- `notes/Product Analyst Intro.docx`: assignment brief and weekly context.
- `notes/project_brief.md`: stakeholder, decision problem, research questions, and analysis structure.
- `daily_time_to_purchase.sql`: main BigQuery query.
- `user_level_time_to_purchase_export.sql`: supporting user-level export query.
...
- `product_analyst_time_to_purchase.pptx`: final PowerPoint presentation.
- `time_to_purchase_dashboard.xlsx`: Excel/Google Sheets-ready visualization workbook.
- `scripts/build_presentation.py`: reproducible script that builds the PowerPoint.
- `scripts/build_excel_visualization.py`: reproducible script that builds the Excel workbook.
- `outputs/archive/`: superseded PowerPoint drafts, planning files, and QA renders.
...
The PowerPoint presentation included with the submission is:
...
outputs/product-analyst-time-to-purchase-submission.zip
```

Older drafts and QA renders are stored separately in `outputs/archive/`.
